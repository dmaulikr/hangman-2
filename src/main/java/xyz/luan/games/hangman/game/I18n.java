...
public final class I18n {
	...
	public static List<String> validLocales() {
		...
		return Arrays.stream(dir.listFiles()).filter(f -> f.getAbsolutePath().endsWith(".json")).map(f -> f.getName().replaceFirst("[.][^.]+$", ""))
	...
	public static String t(String key, String... args) {
		...
		return replaceArgs(i18n, args);
	}

	private static String replaceArgs(String i18n, String[] args) {
		for (int i = 0; i < args.length; i++) {
			i18n = i18n.replaceAll("\\{" + i + "\\}", args[i]);
		}
		return i18n;
	}
}