...
public enum Font {
  GAME_TITLE("GAME_TITLE"), MENU("MENU"), LETTERS("LETTERS"), MANA_COST("MANA_COST"), TITLE("TITLE"), TEXT("TEXT"), TEXT_SMALL("TEXT_SMALL");
  ...
  static class Loader {
    ...
    public static Map<Font, java.awt.Font> loadFontsFromPath(String path) throws BadPackException {
      return PackManager.loadMapFromJson(path + "/fonts.json", (Map<Font, java.awt.Font> map, JsonObject font) -> {
        ...
        if (f == null) {
          return new BadPackException("Invalid font name specified in fonts.json from texturepack/fonts: " + name + ". See the example tp for valid font names and more instructions.");
        }
        ...
        try (InputStream fontStream = Loader.class.getResourceAsStream(path + "/" + file)) {
          ...
          return new BadPackException("Can't find font file specified in fonts.json from texturepack/fonts: " + file + ". Are you sure it exists?", ex);
